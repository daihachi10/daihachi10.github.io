...
function gameOverProcessing() {
    if (countDownTimer <= 0) {
        if (!isStopSounds) { if (isSoundEffect) { isStopSounds = true; stopSounds.play(); } }
        isOdaiShow = false
        nedan = score * 240
        //終了文字
        textSize(45);
        ...
        if (gameOverTien < 0) {
            ...
            if (tobiraX >= 1000) {
                ...
                if (gameOverResultTien > 30) {
                    if (!isResult1Sounds) { if (isSoundEffect) { result1Sounds.play(); isResult1Sounds = true; } }
                    fill("#000");
                ...
                if (gameOverResultTien > 60) {
                    if (!isResult2Sounds) { if (isSoundEffect) { result1Sounds.play(); isResult2Sounds = true; } }
                    fill("#969696");
                    textSize(16)
                    text("5,000円　払って・・・", 130, 115);
                }
                if (gameOverResultTien > 100) {
                    if (!isResult3Sounds) { if (isSoundEffect) { result2Sounds.play(); isResult3Sounds = true; } }
                    if (nedan - 5000 > 0) {
                        kekka = nedan - 5000;
                        stroke("#6f9825");
                        fill("#6f9825");
