document.addEventListener("DOMContentLoaded", () => {
  ...
  const excludeNumbersInput = document.getElementById("exclude-numbers"); // ★ 除外番号入力欄
  ...
  const loadProfiles = () => {
    ...
    if (savedProfiles) {
      ...
      // デフォルトプロファイル
      profiles = [{ id: 1, name: "デフォルト", min: 1, max: 35, exclude: "" }];
      activeProfileId = 1;
    }
    ...
    if (activeProfile) {
      ...
      excludeNumbersInput.value = activeProfile.exclude || ""; // ★ 除外設定を読み込み
    }
    renderProfiles();
  };
  ...
  const addProfile = () => {
    ...
    if (newName && newName.trim() !== "") {
      const newProfile = {
        ...
        exclude: excludeNumbersInput.value, // ★ 除外設定を保存
      };
  ...
  const deleteProfile = (id) => {
    ...
    if (confirm("本当にこのプロファイルを削除しますか？")) {
      ...
      if (activeProfileId === id) {
        ...
        if (activeProfileId) {
          switchProfile(activeProfileId); // ★ switchProfileを呼ぶように変更
        }
      }
      saveProfiles();
      renderProfiles();
  ...
  const switchProfile = (id) => {
    ...
    if (profile) {
      ...
      excludeNumbersInput.value = profile.exclude || ""; // ★ 除外設定を切り替え
      saveProfiles();
      renderProfiles();
  ...
  excludeNumbersInput.addEventListener("change", updateActiveProfile); // ★ イベントリスナー追加
  ...
  contextMenu.addEventListener("click", (e) => {
    ...
    switch (action) {
      ...
      case "save":
        profile.min = parseInt(minNumInput.value);
        profile.max = parseInt(maxNumInput.value);
        profile.exclude = excludeNumbersInput.value; // ★ 除外設定も保存
        saveProfiles();
  ...
  const getAvailableNumbers = () => {
    ...
    for (let i = min; i <= max; i++) {
      if (
        ...
        !excludedNumbers.includes(i) // 除外リストに含まれていないかチェック
      ) {
        available.push(i);
      }
    }
  ...
  const setControlsDisabled = (disabled) => {
    ...
    excludeNumbersInput.disabled = disabled; // ★ 除外設定も無効化
  ...
  const setActiveMode = (mode) => {
    ...
    if (mode === "timer") {
      gameButtons.style.display = "none"; // flexからnoneに変更
      // タイマーモードの初期化
      ...
      gameButtons.style.display = "flex"; // flexに戻す
    }
  ...
  const startRouletteMode = (numbers) => {
    const min = parseInt(minNumInput.value);
    const max = parseInt(maxNumInput.value);
    // 注意: ルーレットの見た目は除外設定を反映しません。抽選結果のみ反映されます。
    const totalNumbersOnWheel = max - min + 1;
    ...
    const resultNumber = numbers[Math.floor(Math.random() * numbers.length)];
    const resultIndexOnWheel = resultNumber - min;
