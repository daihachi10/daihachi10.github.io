...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.body.addEventListener("click", initAudio, { once: !0 });
  document.body.addEventListener("touchstart", initAudio, { once: !0 });
  ...
  const loadItems = () => {
    ...
    if (savedActiveId) {
      ...
      if (
        !items.find((i) => i.id === activeProfileId && i.type === "profile")
      ) {
  ...
  const applyActiveProfileToInputs = () => {
    ...
    if (profile) {
      ...
      useNamesCheckbox.checked = profile.useNames === !0;
  ...
  const setActiveMode = (mode) => {
    ...
    if (mode === "timer") {
      ...
      timerModeTimeLeft = 0;
      updateTimerDisplay();
      timerPauseBtn.disabled = !0;
      timerResetBtn.disabled = !0;
      timerMinutesInput.disabled = !1;
      timerSecondsInput.disabled = !1;
      timerStartBtn.textContent = "スタート";
      clearInterval(timerModeInterval);
      timerModeIsRunning = !1;
    } else {
  ...
  const startRouletteMode = (numbers) => {
    ...
    const duration = parseFloat(1.9) || 5;
  ...
  const startCardMode = (numbers) => {
    ...
    availableCards.forEach((num) => {
      ...
      card.addEventListener("click", handleCardClick, { once: !0 });
  ...
  const startTimerMode = () => {
    ...
    timerModeIsRunning = !0;
    timerStartBtn.textContent = "再開";
    timerPauseBtn.disabled = !1;
    timerResetBtn.disabled = !1;
    timerMinutesInput.disabled = !0;
    timerSecondsInput.disabled = !0;
