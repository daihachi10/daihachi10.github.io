// Service Workerの登録（オフライン対応・キャッシュ高速化）
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const numberSettingsContainer = document.getElementById("number-settings-container");
  const nameSettingsContainer = document.getElementById("name-settings-container");
  ...
  const resultTimerDisplay = document.getElementById("result-timer-display");
  ...
  // --- Web Audio API の実装 (ここから変更) ---
  const AudioContext = window.AudioContext || window.webkitAudioContext;
  const audioCtx = new AudioContext();
  const masterGainNode = audioCtx.createGain();
  masterGainNode.connect(audioCtx.destination);
  masterGainNode.gain.value = 0.4; // 初期音量
  ...
  // ファイルパス定義
  const sounds = {
    simpleTick: "./sounds/simple_tick.mp3",
    simpleResult: "./sounds/simple_result.mp3",
    rouletteTick: "./sounds/roulette_tick.mp3",
    rouletteResult: "./sounds/roulette_result.mp3",
  ...
  const loadSound = async (key, url) => {
    try {
      ...
      console.error(`音声読み込み失敗: ${url}`, e);
    }
  };

  // ページ読み込み時に全音声をキャッシュ（通信はここでの1回のみ）
  Object.entries(sounds).forEach(([key, url]) => {
    // 既存コードとの互換性のため、soundsオブジェクトの値をキー文字列に置換
    sounds[key] = key; 
    loadSound(key, url);
  });
  ...
  const loadItems = () => {
    ...
    if (savedItems) {
      try {
        ...
        items = parsed.map((item) => {
          if (!item.type) item.type = "profile";
          if (item.parentId === undefined) item.parentId = null;
          if (item.type === "folder" && item.isCollapsed === undefined) item.isCollapsed = false;
          return item;
        });
      } catch (e) {
    ...
    if (savedActiveId) {
      ...
      if (!items.find((i) => i.id === activeProfileId && i.type === "profile")) {
  ...
  const createTreeItemElement = (item) => {
    ...
    if (item.type === "folder") {
      ...
      header.innerHTML = ` <span class="folder-arrow">${arrowIcon}</span> <svg class="icon-folder" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M160-160q-33 0-56.5-23.5T80-240v-480q0-33 23.5-56.5T160-800h240l80 80h320q33 0 56.5 23.5T880-640v400q0 33-23.5 56.5T800-160H160Zm0-80h640v-400H447l-80-80H160v480Zm0 0v-480 480Z"/></svg> <span class="folder-name">${item.name}</span> `;
      ...
      header.addEventListener("contextmenu", (e) => showContextMenu(e, item.id));
      actions.querySelector(".menu-btn").addEventListener("click", (e) => {
        e.stopPropagation();
      ...
      content.className = `folder-content ${ item.isCollapsed ? "collapsed" : "" }`;
      content.dataset.folderId = item.id;
      ...
      el.innerHTML = ` <svg class="icon-profile" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M234-276q51-39 114-61.5T480-360q69 0 132 22.5T726-276q35-41 54.5-93T800-480q0-133-93.5-226.5T480-800q-133 0-226.5 93.5T160-480q0 59 19.5 111t54.5 93Zm246-164q-59 0-99.5-40.5T340-580q0-59 40.5-99.5T480-720q59 0 99.5 40.5T620-580q0 59-40.5 99.5T480-440Zm0 360q-83 0-156-31.5T197-197q-54-54-85.5-127T80-480q0-83 31.5-156T197-763q54-54 127-85.5T480-880q83 0 156 31.5T763-763q54 54 85.5 127T880-480q0 83-31.5 156T763-197q-54 54-127 85.5T480-80Zm0-80q53 0 100-15.5t86-44.5q-39-29-86-44.5T480-280q-53 0-100 15.5T294-220q39 29 86 44.5T480-160Zm0-360q26 0 43-17t17-43q0-26-17-43t-43-17q-26 0-43 17t-17 43q0 26 17 43t43 17Zm0-60Zm0 360Z"/></svg> <span class="profile-name" style="flex-grow:1">${item.name}</span> <div class="item-actions"> <button class="action-btn menu-btn"><svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-160q-33 0-56.5-23.5T400-240q0-33 23.5-56.5T480-320q33 0 56.5 23.5T560-240q0 33-23.5 56.5T480-160Zm0-240q-33 0-56.5-23.5T400-480q0-33 23.5-56.5T480-560q33 0 56.5 23.5T560-480q0 33-23.5 56.5T480-400Zm0-240q-33 0-56.5-23.5T400-720q0-33 23.5-56.5T480-800q33 0 56.5 23.5T560-720q0 33-23.5 56.5T480-640Z"/></svg></button> </div> `;
      el.addEventListener("click", () => switchProfile(item.id));
      el.addEventListener("contextmenu", (e) => showContextMenu(e, item.id));
  ...
  function handleDragOver(e) {
    ...
    if (targetType === "folder") {
      ...
      } else if (relY > height - 10 && targetEl.classList.contains("folder-item") && targetEl.querySelector(".folder-content").classList.contains("collapsed")) {
  ...
  function handleDragLeave(e) {
    e.currentTarget.classList.remove("drag-over-top", "drag-over-bottom", "drag-over-folder");
  }
  ...
  function handleDrop(e) {
    ...
    if (targetItem.type === "folder" && relY >= 10 && !(relY > rect.height - 10 && items[targetIndex].isCollapsed)) {
  ...
  const addNewItem = (type) => {
    const name = prompt(type === "profile" ? "プロファイル名:" : "フォルダ名:", type === "profile" ? "新規プロファイル" : "新規フォルダ");
  ...
  contextMenu.addEventListener("click", (e) => {
    ...
    if (action === "load") {
      ...
      if (newName && newName.trim() !== "") {
        ...
        if (item.id === activeProfileId) profileNameDisplay.textContent = item.name;
        saveItems();
        renderTree();
      }
  ...
  const getAvailableNumbers = () => {
    ...
    const excludedNumbers = excludeString.split(",").map((n) => parseInt(n.trim())).filter((n) => !isNaN(n));
    const available = [];
    for (let i = min; i <= max; i++) {
      if ((!noDuplicatesCheckbox.checked || !drawnNumbers.includes(i)) && !excludedNumbers.includes(i)) {
        available.push(i);
      }
    }
    return available;
  ...
  const rouletteAudioTick = () => {
    ...
    if (transform !== "none") {
      ...
      while (rouletteAudioContext.angleSinceLastTick >= rouletteAudioContext.segmentAngle) {
        rouletteAudioContext.angleSinceLastTick -= rouletteAudioContext.segmentAngle;
        const now = Date.now();
        if (now - rouletteAudioContext.lastTickTimestamp > rouletteAudioContext.minTimeBetweenTicks) {
  ...
  const startRouletteAudio = (segmentAngle) => {
    ...
    rouletteAudioContext = { ...rouletteAudioContext,
      loopId: null,
  ...
  startButton.addEventListener("click", () => {
    if (currentMode === "roulette" && isSpinning) {
      ...
      setTimeout(() => {
        rouletteWheel.style.transition = "transform 5s cubic-bezier(0.1, 0.8, 0.2, 1)";
      }, 1500);
  ...
  const startSimpleMode = (numbers) => {
    ...
    const spinInterval = setInterval(() => {
      resultDisplay.textContent = numbers[Math.floor(Math.random() * numbers.length)];
  ...
  const startRouletteMode = (numbers) => {
    ...
    for (let i = 0; i < totalNumbersOnWheel; i++) {
      ...
      gradient += `${colors[i % colors.length]} ${i * segmentAngle}deg ${ (i + 1) * segmentAngle }deg`;
      if (i < totalNumbersOnWheel - 1) gradient += ", ";
      const numberDiv = document.createElement("div");
      numberDiv.className = "number";
      numberDiv.style.transform = `rotate(${ i * segmentAngle + segmentAngle / 2 }deg)`;
    ...
    const targetSegmentCenterAngle = resultIndexOnWheel * segmentAngle + segmentAngle / 2;
  ...
  const handleCardClick = (e) => {
    ...
    const resultValue = isNaN(resultNumber) ? clickedCard.dataset.number : resultNumber;
  ...
  resetButton.addEventListener("click", () => {
    ...
    availableCards = [];
    startButton.textContent = currentMode === "card" ? "カードを準備" : "スタート！";
    if (currentMode === "card") {
      const availableNumbers = getAvailableNumbers();
      if (availableNumbers && availableNumbers.length > 0) {
        startCardMode(availableNumbers);
